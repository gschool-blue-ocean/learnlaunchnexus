...
const Dashboard = ({ setAuth, userEmail }) => {
	...
	const getProfile = async (EMAIL) => {
		try {
			const res = await fetch(
				`${import.meta.env.VITE_API}/users/init/${EMAIL}`,
				{
					method: "GET",
				}
			);
			...
			if (
				parseData.github_acct === null ||
				parseData.github_acct === undefined
			) {
			} else {
	...
	const getLocation = async (USER_ID) => {
		try {
			const res = await fetch(
				`${import.meta.env.VITE_API}/students/${USER_ID}`,
				{
					method: "GET",
				}
			);
							...
							<div className='profile'>
								<div className='header'>
									<img
										...
										src={`https://avatars.githubusercontent.com/${gitPic}`}
									></img>
									<h1>Welcome, {name} !</h1>

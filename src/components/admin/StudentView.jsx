...
const StudentView = ({ statusList, students, studentID, onBack }) => {
	...
	const handleSubmitFeedback = async (e) => {
		...
		try {
			const res = await fetch(
				`${import.meta.env.VITE_API}/submission/${feedbackName}`,
				{
					method: "PATCH",
					headers: {
	...
	const handleStatus = async (e) => {
		...
		try {
			const res = await fetch(
				`${import.meta.env.VITE_API}/submission/${setSubmission}`,
				{
					method: "PATCH",
					headers: {
						Accept: "application/json",
			...
			if (selectedStudentId !== null && selectedStudentId !== "0") {
				try {
					const response = await fetch(
						...
						}/submission/student/${selectedStudentId}`
					);
				...
				<tbody>
					...
					{submissions.map((submission) => (
						<tr key={submission.submission_id}>
							...
							<td>
								<button name={submission.submission_id} onClick={handleStatus}>
									Confirm
								</button>
							</td>

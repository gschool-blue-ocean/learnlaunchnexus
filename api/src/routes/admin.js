...
router.patch("/add/:email", async (req, res) => {
	...
	try {
		const result = await pool.query(
			"UPDATE Users set isAdmin = true WHERE auth_id = (SELECT user_id from Authentication where user_email = $1) returning *; ",
			[email]
		);
		if (result.rows.length === 0)
...
router.patch("/delete/:email", async (req, res) => {
	...
	try {
		const result = await pool.query(
			"UPDATE Users set isAdmin = false WHERE auth_id = (SELECT user_id from Authentication where user_email = $1) returning *; ",
			[email]
		);
		if (result.rows.length === 0)
			return res.status(404).json({ message: "User not found." });
...
router.put("/:id", async (req, res) => {
	...
	try {
		const result = await pool.query(
			"UPDATE admin SET user_id = $1 WHERE id = $2 RETURNING *",
			[user_id, id]
		);
		if (result.rows.length === 0)
			return res.status(404).json({ message: "Admin not found." });

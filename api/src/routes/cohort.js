...
router.post("/", async (req, res) => {
	...
	try {
		const result = await pool.query(
			"INSERT INTO cohort (start_date, end_date, name) VALUES ($1, $2, $3) RETURNING *",
...
router.put("/:id", async (req, res) => {
	...
	try {
		const result = await pool.query(
			"UPDATE cohort SET start_date = $1, end_date = $2, name = $3 WHERE id = $4 RETURNING *",

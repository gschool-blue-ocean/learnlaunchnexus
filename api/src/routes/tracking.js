...
router.get("/:id", async (req, res) => {
	const { id } = req.params;
	try {
		const result = await pool.query("SELECT * FROM tracking WHERE id = $1", [
			id,
		]);
...
router.post("/", async (req, res) => {
	...
	try {
		const result = await pool.query(
			"INSERT INTO tracking (status) VALUES ($1) RETURNING *",
			[status]
		);
		res.json(result.rows[0]);
	} catch (err) {
...
router.put("/:id", async (req, res) => {
	...
	try {
		const result = await pool.query(
			"UPDATE tracking SET status = $1 WHERE id = $2 RETURNING *",
			[status, id]
		);
		if (result.rows.length === 0)
			return res.status(404).json({ message: "Tracking status not found." });

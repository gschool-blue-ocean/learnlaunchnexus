...
router.get("/:id", async (req, res) => {
	...
	try {
		const result = await pool.query(
			"SELECT * FROM cohort_assignment WHERE id = $1",
			[id]
		);
		if (result.rows.length === 0)
			return res
				.status(404)
				.json({ message: "Cohort-Assignment mapping not found." });
...
router.get("/cohort/:id", async (req, res) => {
	...
	try {
		const result = await pool.query(
			"SELECT assignment_id, name FROM cohort_assignment INNER JOIN assignment ON cohort_assignment.assignment_id=assignment.id WHERE cohort_id = $1 ORDER BY assignment_id ASC",
			[id]
		);
		if (result.rows.length === 0)
			return res
...
router.post("/", async (req, res) => {
	...
	try {
		const result = await pool.query(
			"INSERT INTO cohort_assignment (assignment_id, cohort_id) VALUES ($1, $2) RETURNING *",

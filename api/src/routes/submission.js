...
router.get("/:id", async (req, res) => {
	const { id } = req.params;
	try {
		const result = await pool.query("SELECT * FROM submission WHERE id = $1", [
			id,
		]);
...
router.post("/", async (req, res) => {
	...
	try {
		const result = await pool.query(
			"INSERT INTO submission (info, submission_time, student_id, assignment_id, tracking_id, feedback) VALUES ($1, NOW(), $2, $3, $4, $5) RETURNING *",
...
router.patch("/:id", async (req, res) => {
	...
	try {
		...
		const SQLString = format(
			"UPDATE submission SET %s WHERE id = %L RETURNING *",
			setStrings,
			id
		);
...
router.put("/:id", async (req, res) => {
	...
	try {
		const result = await pool.query(
			"UPDATE submission SET info = $1, student_id = $2, assignment_id = $3, tracking_id = $4, feedback = $6 WHERE id = $5 RETURNING *",
